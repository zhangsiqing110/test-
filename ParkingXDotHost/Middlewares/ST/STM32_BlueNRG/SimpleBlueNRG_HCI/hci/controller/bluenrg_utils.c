...
#include "osal.h"
#include "string.h"
#ifdef USE_RTI_XDOT_PARKING_MODULE
...
/* This function calculates the CRC of a sector of flash, since pni store BLE image in the
   external SPI flash, we can only read in pieces of BLE firmware at a time.
*/
static uint32_t pni_updater_calc_crc(uint32_t startAddr, uint16_t nr_of_bytes)
{
    uint32_t i, j, a1;
    uint32_t crc, value;

    crc = 0;
    //printf("[DBG]start addr = 0x%.08X\r\n",startAddr);
    for (i = 0; i < SECTOR_SIZE; i += 4) {
      uint8_t *dataw = (uint8_t *) &value;
      sFLASH_ReadBuffer(dataw, startAddr + i, 4);
      ...
      //printf("[DBG]crc = 0x%.08X, data[0x%.08X]=0x%.02X 0x%.02X 0x%.02X 0x%.02X \r\n",
      //  crc, spiAddr + i, data[0], data[1], data[2], data[3]);
    }
    return crc;
}

int pni_program_device(uint32_t spiAddr, uint32_t fw_size)
{
  uint8_t version, num_erase_retries=0, status, data_size;
  uint8_t number_sectors, module;
  uint32_t address, j;//, spiAddr;
  uint8_t bleSpiFlash[DATA_SIZE];
  uint32_t crc, crc2, crc_size;
  uint32_t fw_offset = FW_OFFSET;
  printf("Upgrade BLE Stack...\r\n");
  ...
  printf("\tBLE booloader version: %d\r\n", version);
  ...
  ************************************************************************/
  printf("\tErase and Program Sectors...");
  //spiAddr = PNI_FOTA_EXTFLAH_BLE1_ADDR_START;
  for(int i = fw_offset; i < (number_sectors * SECTOR_SIZE); i += SECTOR_SIZE) {
    ...
    while (num_erase_retries++ < MAX_ERASE_RETRIES) {
      ...
      for (j=i; ((j<SECTOR_SIZE+i)&&(j<fw_size)); j += data_size) {
        sFLASH_ReadBuffer(bleSpiFlash, spiAddr + j, data_size);
        #if 0 // enable to verify data
        printf("[0x%.08X:]", BASE_ADDRESS+j);

        //address = address + 16;
        for (int cnt = 0; cnt < data_size; cnt++)
        {
          printf("0x%.02X ", bleSpiFlash[cnt]);
        }
        printf("\r\n");
        ...
        RETRY_COMMAND(aci_updater_program_data_block(BASE_ADDRESS+j, data_size, bleSpiFlash), MAX_WRITE_RETRIES, status);
        if (status != BLE_STATUS_SUCCESS)
          break;
      }
  ...
  ************************************************************************/
  ...
  for(int i = fw_offset; i < (number_sectors*SECTOR_SIZE); i += SECTOR_SIZE){
    ...
    crc2 = pni_updater_calc_crc(spiAddr + i,crc_size);
    //printf("final crc2 = 0x%.08X\r\n", crc2);
...
uint8_t getBlueNRGVersion(uint8_t *hwVersion, uint16_t *fwVersion)
{
  ...
  printf("hci_le_read_local_version(%u)\n", status);
  ...
  {
    printf("[ERROR] read BLE version(%u)\n", status);
  }
