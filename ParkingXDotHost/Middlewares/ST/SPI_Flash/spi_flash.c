...
#define SPI_FLASH_DBG  1
#else
#define SPI_FLASH_DBG  0
#endif
...
void sFLASH_Init(void)
{
...
#if SPI_FLASH_DBG
  PNI_PRINTF("Detecting SPI Flash...");
#endif
  ...
  {
    PNI_PRINTF("Found(M25P64)\r\n");
  }
  else if (FlashID == sFLASH_M25P16_ID)
  {
    PNI_PRINTF("Found(M25P16)\r\n");
  }
  else if (FlashID == sFLASH_S25FL116K_ID)
  {
    PNI_PRINTF("Found(S25FL116K)\r\n");
  }
  else if (FlashID == sFLASH_S25FL132K_ID)
  {
    PNI_PRINTF("Found(S25FL132K)\r\n");
  }
  else if (FlashID == sFLASH_S25FL164K_ID)
  {
    PNI_PRINTF("Found(S25FL164K)\r\n");
  }
  else
  {
    PNI_PRINTF("[ERROR] Not Found(flash ID = 0x%.08X)\r\n", FlashID);
  }
#endif
...
{
  uint8_t txBuf = sFLASH_CMD_RDID;
  uint8_t rxBuf[4] = {0xaa, 0x00, 0x00, 0x00};
  uint32_t flashId = 0;

  /*!< Select the FLASH: Chip Select low */
  sFLASH_CS_LOW();

  HAL_SPI_TransmitReceive(&hspi2, (uint8_t*)&txBuf, (uint8_t *)rxBuf, sizeof(rxBuf), 5000);

  /*!< Deselect the FLASH: Chip Select high */
  sFLASH_CS_HIGH();

  // <manufacturer id(1byte)><device id(2bytes)
  flashId = (rxBuf[1] << 16) | (rxBuf[2] << 8) | rxBuf[3];
...
{
  uint8_t rxBuf;
  HAL_SPI_TransmitReceive(&hspi2, (uint8_t*)&byte, (uint8_t *)&rxBuf, 1, 5000);
...
void sFLASH_LowLevel_Init(void)
{
  // using SPI2 to communicate with SPI flash
#if SPI_FLASH_DBG
  PNI_PRINTF("Initialize SPI2...");
#endif
